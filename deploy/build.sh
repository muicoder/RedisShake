...
VERSION="$(curl -fsSLk "https://api.github.com/repos/alibaba/RedisShake/releases/latest" | grep .tag_name | awk -F\" '{print $(NF-1)}')"
...
[ -s jyt ] || docker run --rm --entrypoint sh -v "$PWD:/pwd" -w /pwd muicoder/redis-shake:v4 -c 'cp -av /usr/bin/jyt .'
...
docker build --network host -t muicoder/redis-shake:v4 .
